...
statements: statement
    ...
    | statements RETURN value SEMICOLON
    {
        char *result = malloc(strlen($1) + strlen($3) + 10);
        sprintf(result, "%sreturn %s\n", $1, $3);
        $$ = result;
    }
    | statements RETURN VARIABLE SEMICOLON
    {
        char *result = malloc(strlen($1) + strlen($3) + 10);
        sprintf(result, "%sreturn %s\n", $1, $3 + 1);  // Skip the '$' character
        $$ = result;
    }
    ;
...
{
    char *result = malloc(strlen($1) + strlen($3) + 2);
    sprintf(result, "%s, %s", $1, $3 + 1);  // Skip the '$' character
    $$ = result;
}
...
{
    char *func_name = $1;  // Skip the '$' character
    char *args = $3;
